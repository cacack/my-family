...
// Projector against the sqlite backend. The scenario body (runBranchScenario) is an
// identical copy of the memory/postgres versions so all three backends prove the
// same overlay / tombstone / fallback / purge behavior (DB-001). Fixtures use
// neutral placeholder names only (public repo — no real PII).
func TestBranchScenario_EndToEnd(t *testing.T) {
	readStore, branchStore, cleanup := setupBranchScenarioStores(t)
	defer cleanup()
	runBranchScenario(t, readStore, branchStore)
}

// setupBranchScenarioStores builds a ReadModelStore and a BranchStore over one
// shared sqlite database (a temp file, so both stores see the same tables).
func setupBranchScenarioStores(t *testing.T) (*sqlite.ReadModelStore, *sqlite.BranchStore, func()) {
	t.Helper()

	tmpFile, err := os.CreateTemp("", "myfamily-branch-scenario-*.db")
	if err != nil {
	...
	return readStore, branchStore, func() {
		db.Close()
		os.Remove(tmpFile.Name())
	}
}
...
// sqlite backend: a mainline browse must be unaffected by the existence of branch
...
// the memory and postgres versions so all three backends prove the same behavior
