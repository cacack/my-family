...
// Projector against the postgres backend. The scenario body (runBranchScenario) is
// an identical copy of the memory/sqlite versions so all three backends prove the
// same overlay / tombstone / fallback / purge behavior (DB-001). It stays behind the
// testing.Short() + isDockerAvailable() skips (via setupPostgres) so CI without
// Docker still passes. Fixtures use neutral placeholder names only (public repo —
// no real PII).
...
// postgres backend: a mainline browse must be unaffected by the existence of branch
...
// the memory and sqlite versions so all three backends prove the same behavior
// (DB-001). It stays behind the same testing.Short() + isDockerAvailable() skips as
// TestBranchScenario_EndToEnd, so CI without Docker still passes — which also means a
// Docker-less run verifies this scenario on memory and sqlite only. Fixtures use
// neutral placeholder names and invented places only (public repo — no real PII).
