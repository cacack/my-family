...
func TestBranchDeletePersonCascadesTombstones(t *testing.T) {
	...
	if err := store.SavePerson(ctx, domain.MainBranchID, pgPersonRM(personID, "Cascade", "Main")); err != nil {
		t.Fatalf("SavePerson: %v", err)
	}
	if err := store.SavePersonName(ctx, domain.MainBranchID, &repository.PersonNameReadModel{ID: uuid.New(), PersonID: personID, GivenName: "Cascade", Surname: "Main"}); err != nil {
		t.Fatalf("SavePersonName: %v", err)
	}
	if err := store.ReplacePersonExternalIDs(ctx, domain.MainBranchID, personID, []repository.PersonExternalIDReadModel{{Value: "X"}}); err != nil {
	...
	// Every read is error-checked: a failing query must not pass vacuously by
	// returning an empty slice.
	branchNames, err := store.GetPersonNames(ctx, branch, personID)
	if err != nil {
		t.Fatalf("GetPersonNames (branch): %v", err)
	}
	if len(branchNames) != 0 {
		t.Fatalf("branch names after cascade tombstone: want empty, got %+v", branchNames)
	}
	branchIDs, err := store.GetPersonExternalIDs(ctx, branch, personID)
	if err != nil {
		t.Fatalf("GetPersonExternalIDs (branch): %v", err)
	}
	if len(branchIDs) != 0 {
		t.Fatalf("branch ext ids after cascade tombstone: want empty, got %+v", branchIDs)
	}
	mainNames, err := store.GetPersonNames(ctx, domain.MainBranchID, personID)
	if err != nil {
		t.Fatalf("GetPersonNames (main): %v", err)
	}
	if len(mainNames) != 1 {
		t.Fatalf("main names after branch cascade: want 1, got %+v", mainNames)
	}
	mainIDs, err := store.GetPersonExternalIDs(ctx, domain.MainBranchID, personID)
	if err != nil {
		t.Fatalf("GetPersonExternalIDs (main): %v", err)
	}
	if len(mainIDs) != 1 {
		t.Fatalf("main ext ids after branch cascade: want 1, got %+v", mainIDs)
	}
}

// TestBranchFamilyOverlayTombstone mirrors the SQLite test of the same name so
// both backends assert identical family overlay behaviour (DB-001): branch row
// shadows main, ListFamilies is branch-scoped, and a branch delete tombstones the
// family for the branch while main keeps it.
func TestBranchFamilyOverlayTombstone(t *testing.T) {
	...
	if err := store.SaveFamily(ctx, domain.MainBranchID, &repository.FamilyReadModel{ID: id, Partner1GivenName: "Main", Version: 1}); err != nil {
		t.Fatalf("SaveFamily main: %v", err)
	}
	if err := store.SaveFamily(ctx, branch, &repository.FamilyReadModel{ID: id, Partner1GivenName: "Branch", Version: 1}); err != nil {
		t.Fatalf("SaveFamily branch: %v", err)
	}

	got, err := store.GetFamily(ctx, branch, id)
	if err != nil {
		t.Fatalf("GetFamily branch: %v", err)
	}
	if got == nil || got.Partner1GivenName != "Branch" {
		t.Fatalf("branch GetFamily: want Branch, got %+v", got)
	}
	got, err = store.GetFamily(ctx, domain.MainBranchID, id)
	if err != nil {
		t.Fatalf("GetFamily main: %v", err)
	}
	if got == nil || got.Partner1GivenName != "Main" {
		t.Fatalf("main GetFamily: want Main, got %+v", got)
	}

	branchList, total, err := store.ListFamilies(ctx, repository.ListOptions{Limit: 10, BranchID: branch})
	if err != nil {
	...
	got, err = store.GetFamily(ctx, branch, id)
	if err != nil {
		t.Fatalf("GetFamily branch after tombstone: %v", err)
	}
	if got != nil {
	...
	got, err = store.GetFamily(ctx, domain.MainBranchID, id)
	if err != nil {
		t.Fatalf("GetFamily main after branch tombstone: %v", err)
	}
	if got == nil {
...
// TestBranchFamilyExternalIDsOverlay mirrors the SQLite test of the same name:
// a branch's replace-all bucket overrides main, and a branch that never touched
// the family falls back to main's identifiers.
func TestBranchFamilyExternalIDsOverlay(t *testing.T) {
	...
	if err := store.ReplaceFamilyExternalIDs(ctx, domain.MainBranchID, familyID, []repository.FamilyExternalIDReadModel{{Value: "FMAIN"}}); err != nil {
		t.Fatalf("ReplaceFamilyExternalIDs main: %v", err)
	}
	if err := store.ReplaceFamilyExternalIDs(ctx, branch, familyID, []repository.FamilyExternalIDReadModel{{Value: "FBR-1"}, {Value: "FBR-2"}}); err != nil {
		t.Fatalf("ReplaceFamilyExternalIDs branch: %v", err)
	}

	branchIDs, err := store.GetFamilyExternalIDs(ctx, branch, familyID)
	if err != nil {
		t.Fatalf("GetFamilyExternalIDs branch: %v", err)
	}
	...
	fallbackIDs, err := store.GetFamilyExternalIDs(ctx, other, familyID)
	if err != nil {
		t.Fatalf("GetFamilyExternalIDs fallback: %v", err)
	}
