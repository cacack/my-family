<script lang="ts">
	...
	async function loadPersons() {
		loading = true;
		try {
			const result = await api.getPersonsByPlace(place, {
				limit: pageSize,
...
<svelte:head>
	<title>{place} | Browse Places | My Family</title>
</svelte:head>

<div class="place-detail-page">
	<header class="page-header">
		<nav class="breadcrumb">
			<a href="/browse/places">Browse Places</a>
			<span class="separator">/</span>
			<span class="current">{place}</span>
		</nav>
		<h1>People from "{place}"</h1>
		{#if total > 0}
	...
	{:else if persons.length === 0}
		<div class="empty">
			<p>No people found from "{place}".</p>
			<a href="/browse/places" class="back-link">Back to place browser</a>
		</div>
	{:else}
